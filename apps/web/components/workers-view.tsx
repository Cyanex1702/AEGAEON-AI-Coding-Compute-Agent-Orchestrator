...
import { Cpu, HardDrive, MemoryStick, Microchip, Radio, Server } from "lucide-react";
import type { Worker } from "@/lib/types";
import { Badge, MiniBar, StatusDot, timeAgo } from "./ui";
  ...
  return (
    <div className="page-wrap">
      ...
      <section className="panel overflow-hidden">
        ...
        {workers.length ? <div className="workers-table"><div className="workers-table-head"><span>Worker</span><span>Status</span><span>Compute</span><span>Utilization</span><span>Models</span><span>Heartbeat</span></div>{workers.map((worker) => <div className="workers-table-row" key={worker.id}><div className="min-w-0"><div className="truncate text-sm font-medium text-zinc-200">{worker.hostname}</div><div className="mt-1 truncate font-mono text-[10px] text-zinc-700">{worker.id}</div></div><div><Badge tone={worker.status === "offline" ? "danger" : worker.status === "busy" ? "cyan" : "acid"}><StatusDot status={worker.status} pulse={worker.status === "busy"} />{worker.status}</Badge></div><div><div className="text-xs text-zinc-300">{worker.hardware.gpu?.available ? worker.hardware.gpu.name : `${worker.hardware.cpu_cores} CPU cores`}</div><div className="mt-1 text-[10px] text-zinc-600">{Math.round((worker.hardware.ram_mb ?? 0) / 1024)} GB RAM{worker.hardware.gpu?.available ? ` · ${Math.round((worker.hardware.gpu.vram_mb ?? 0) / 1024)} GB VRAM` : ""}</div></div><div className="space-y-2"><Usage label="CPU" value={worker.cpu_percent} /><Usage label="RAM" value={worker.ram_percent} /></div><div className="tag-list">{worker.models.length ? worker.models.map((model) => <span key={model.id} title={model.last_error ?? undefined}>{model.id}{model.runtime ? ` · ${model.runtime}` : ""}{model.quantization ? ` · ${model.quantization}` : ""}{model.status ? ` · ${model.status}` : ""}</span>) : <span>runtime only</span>}</div><div className="text-xs text-zinc-600">{timeAgo(worker.last_heartbeat)}</div></div>)}</div> : <div className="empty-worker-state"><div className="empty-worker-icon"><Cpu className="h-6 w-6" /></div><h3>No external workers connected</h3><p>Create a Broke Boy project to generate secure Run-All Colab notebooks, or start a local worker.</p><code>python -m worker.worker --controller ws://127.0.0.1:8000/ws/worker</code></div>}
      </section>
    </div>
  );
}
